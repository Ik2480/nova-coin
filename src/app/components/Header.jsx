...
export default function Header() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-shadow duration-200 bg-gray-800 ${
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-3">
              ...
              <span className="hidden sm:inline-block font-semibold text-lg tracking-wide text-white">
                Novetoken
              </span>
            </Link>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-6" aria-label="Primary">
            {NAV.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`relative px-2 py-1 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors text-gray-300 hover:text-white ${
                  isActive(item.href)
                    ? "text-indigo-400"
                    : ""
                }`}
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:inline-flex">
              <button
                ...
                className="p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-white"
                onClick={toggleTheme}
                title="Toggle theme"
              >
            ...
            <button
              ...
              className="inline-flex items-center justify-center p-2 rounded-md md:hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-white"
            >
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              className="absolute right-0 top-0 h-full w-full max-w-xs bg-gray-800 shadow-xl p-6 flex flex-col"
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-gray-900 text-white dark:bg-white dark:text-gray-900 font-semibold">NT</span>
                  <span className="font-semibold text-white">Novetoken</span>
                </div>
                <button
                  onClick={() => setOpen(false)}
              ...
              <nav className="flex-1 flex flex-col gap-3" aria-label="Mobile primary">
                {NAV.map((item, i) => (
                  <Link
                    ...
                    onClick={() => setOpen(false)}
                    className={`block px-3 py-2 rounded-lg text-base font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors text-gray-300 hover:text-white ${
                      isActive(item.href)
